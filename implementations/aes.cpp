...
class LookupTables {
    public:
        ...
        int mix_columns_coefficients[4][4] = {
            {2, 3, 1, 1},
            {1, 2, 3, 1},
            {1, 1, 2, 3},
            {3, 1, 1, 2}
...
void g_function(unsigned char *word, int num_round) {
    ...
    //S-Box substitution with Round Coefficient in byte 0
    word[0] = ltables.s_box[word[0]] ^ ltables.round_coefficient[num_round - 1];
    for (int i = 1; i < 4; i++) {
        word[i] = ltables.s_box[word[i]];
    }
}

void build_state_matrix(unsigned char **state_matrix, unsigned char *plaintext) {
    ...
    for (int j = 0; j < N_STATE_MATRIX; j++) {
        for (int i = 0; i < N_STATE_MATRIX; i++) {
            state_matrix[i][j] = plaintext[(j * N_STATE_MATRIX) + i];
        }
    }
}
...
void byte_substitution(unsigned char **state_matrix) {
    LookupTables ltables;

    // First layer of each round. Is based on the S-Boxes
    for (int j = 0; j < N_STATE_MATRIX; j++) {
        for (int i = 0; i < N_STATE_MATRIX; i++) {
            state_matrix[j][i] = ltables.s_box[state_matrix[j][i]];
        }
    }
}
...
void shift_rows(unsigned char **state_matrix) {
    ...
    unsigned char temp = state_matrix[1][0];
    ...
    unsigned char temp1 = state_matrix[2][1];
    state_matrix[2][0] = state_matrix[2][2];
    state_matrix[2][1] = state_matrix[2][3];
    state_matrix[2][2] = temp;
    state_matrix[2][3] = temp1;

    // Fourth row is shifted three positions to the left
    temp = state_matrix[3][0];
    temp1 = state_matrix[3][1];
    unsigned char temp2 = state_matrix[3][2];
    state_matrix[3][0] = state_matrix[3][3];
    state_matrix[3][1] = temp;
    state_matrix[3][2] = temp1;
...
void mix_single_column(unsigned char **state_matrix, unsigned char *res_column, int column) {
    ...
    // Copying the resulting column to the state matrix
    for (int i = 0; i < N_STATE_MATRIX; i++) {
        state_matrix[i][column] = res_column[i];
    }
}
...
void key_addition(unsigned char **state_matrix, unsigned char **round_keys, int round) {
    // Both matrices are column-major ordered

    for (int i = 0; i < N_STATE_MATRIX; i++) {
        for (int j = 0; j < N_STATE_MATRIX; j++) {
            state_matrix[i][j] ^= round_keys[round][i + 4 * j];
        }
    }
}
